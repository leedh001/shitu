...
class SetupMainWindow:
    ...
    # TITLE BAR — Leading (leftmost) / Trailing (before window buttons)
    # ///////////////////////////////////////////////////////////////
    add_title_bar_left_menus = [
        {
            "btn_icon" : "icon_folder_open.svg",
            "btn_id" : "btn_workspace",
            "btn_tooltip" : "工作区 / 文件列表",
            "is_active" : False
        },
    ]

    add_title_bar_right_menus = [
        {
    ...
    def setup_gui(self):
        ...
        # ///////////////////////////////////////////////////////////////
        # ADD MENUS
        self.ui.title_bar.add_left_menus(SetupMainWindow.add_title_bar_left_menus)
        self.ui.title_bar.add_right_menus(SetupMainWindow.add_title_bar_right_menus)
        ...
        # SET INITIAL PAGE（左/右侧栏菜单在加载主题并挂载工作区后再设）
        # ///////////////////////////////////////////////////////////////
        ...
        # RIGHT COLUMN — menu_1：工作区文件树（toggle_right_column，布局上在内容区左侧）
        ...
        vl = self.ui.right_column.verticalLayout
        while vl.count():
            item = vl.takeAt(0)
            w = item.widget()
            if w is not None:
                w.setParent(None)
                w.deleteLater()
        vl.addWidget(self.workspace_panel, 1)
        self.workspace_panel.workspace_folder_chosen.connect(self.load_images_from_directory)
        ...
        # 启动时与工作区侧栏打开状态一致：展开 right_column、显示 menu_1、点亮 btn_workspace
        wmax = self.settings["right_column_size"]["maximum"]
        self.ui.right_column_frame.setMinimumWidth(wmax)
        self.ui.right_column_frame.setMaximumWidth(wmax)
        MainFunctions.set_right_column_menu(self, self.ui.right_column.menu_1)
        btn_ws = MainFunctions.get_title_bar_btn(self, "btn_workspace")
        if btn_ws is not None:
            btn_ws.set_active(True)
        btn_top = MainFunctions.get_title_bar_btn(self, "btn_top_settings")
        if btn_top is not None:
            btn_top.set_active(False)
